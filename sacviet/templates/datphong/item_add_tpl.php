...
<script language="javascript">
	function cblist_onchange()
	{
		...
		window.location ="index.php?com=datphong&act=<?=@$_GET['act']?>&id_list="+a.value+"&id_cat=<?=@$_GET['id_cat']?>&id=<?=@$_GET['id']?>";	
		return true;
	}
...
function get_main_list()
	{
		$sql="select * from table_datphong_list order by stt";
...
function get_main_cat()
	{
		$sql="select * from table_datphong_cat";
...
?>
<form name="frm" method="post" action="index.php?com=datphong&act=save&curPage=<?=@$_REQUEST['curPage']?>" enctype="multipart/form-data" class="nhaplieu">	 
    <b>Danh mục cấp 1:</b><?=get_main_list();?><br /><br />  
    <b>Danh mục cấp 2:</b><?=get_main_cat();?><br /><br />
	<?php if (@$_REQUEST['act']=='edit')
	{?>
	<b>Hình hiện tại:</b><img src="<?=_upload_datphong.$item['thumb']?>"  alt="NO PHOTO" /><br />
	<?php }?>
	<b>Hình ảnh:</b> <input type="file" name="file" /> Width:1280px&nbsp;Height:1024px&nbsp;<?=_datphong_type?><br />
    <br />
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=datphong&act=man'" class="btn" />
</form>