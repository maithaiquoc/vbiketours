...
<script language="javascript">				
	...
	{				
		...
		window.location ="index.php?com=news&act=<?php if($_REQUEST['act']=='edit') echo 'edit'; else echo 'add';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_cat="+a.value;	
...
var id_cat= document.getElementById("id_cat");
        if(id_item.value == "-1" || id_cat.value == "-1" ){
			alert("Hãy chọn danh mục trước khi thêm.");
            id_item.focus();		
			return false;
	    }else
		{
			...
			if(item_ten.value == ""){
				alert("Hãy Tiêu đề trước khi thêm.");
				item_ten.focus();		
				return false;
			}else
			{
...
function get_main_category()

	{
		...
		$sql_huyen="select * from table_news_cat order by id desc ";
...
<form name="frm" id="frm" method="post" action="index.php?com=news&act=save" enctype="multipart/form-data" class="nhaplieu">  
...
<b>Danh mục:</b><?=get_main_item();?><br /><br />
   ...
   <?php if (@$_REQUEST['act']=='edit')
	...
	<b>Hình ảnh:</b> <input type="file" name="file" />Width: 120px - Height: 80px <?=_news_type?><br /><br />
...
<td style="width:5%;" align="center"><input type="checkbox" name="check_list[]" value="<?=$items[$i]['id']?>" <?php if($items[$i]['new_id'] != '') echo 'checked';  ?> /></td>
</tr>
<?php	}?>
</table>
