...
function get_items(){
	...
	$sql = "select * from #_user where username <> 'admin' order by username";
...
function save_item(){
	...
	if($id){ // cap nhat
		...
		$sql_user="select * from table_user where username = '".$_POST['username']."' and id <> '".$id."'";
		...
		if($d->insert($data))
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=user&act=add");
