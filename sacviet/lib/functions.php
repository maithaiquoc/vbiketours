...
//$default_max_file_size = ini_get('upload_max_filesize');alert($default_max_file_size);
//alert($default_max_file_size);
//alert(isset($_FILES[$file]));
//$_FILES[$file]['error'];
//alert($_FILES[$file]['size']);
//alert($_FILES["file"]["name"].$_FILES["file"]["type"].$_FILES["file"]["size"].$_FILES["file"]["tmp_name"].$_FILES["file"]["error"]);
//alert(php_ini_loaded_file() . '<br />'. ini_get('upload_max_filesize'));
//$filename = basename($_FILES[$file]['name'], '.'.$ext);
//alert($filename . ': ' . filesize($filename) . ' bytes');
...
//ini_set('upload_max_filesize', '64M');
	...
	if(isset($_FILES[$file]) && !$_FILES[$file]['error']){
		...
		if(strpos($extension, $ext)===false){
			...
			echo "<script>alert('Chỉ hỗ trợ hỗ trợ upload file dạng ".$extension."');</script>";
...

	{
...

		{
			...
			#{
				...
				$paging .=" <a href='".$url."&curPage=".($totalPages)."' class=\"{$class_paging}\" ><<</a> "; //ve cuoi
...
function showDigits($digits='00000'){
		...
		foreach($digits as $digit){
			...
			$ret .= '<img src="images/counter/'.$digit.'.jpg" />';
