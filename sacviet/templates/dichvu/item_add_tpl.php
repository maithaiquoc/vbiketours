...
<script language="javascript">				
	...
	{				
		...
		window.location ="index.php?com=dichvu&act=<?php if($_REQUEST['act']=='edit') echo 'edit'; else echo 'add';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_cat="+a.value;	
		return true;
	}
...
function get_main_category()
	{
		$sql_huyen="select * from table_dichvu_item order by id desc ";
...
function get_main_item()
	{
		$sql_huyen="select * from table_dichvu_item where id_cat='".$_REQUEST["id_cat"]."' order by id desc ";
		$result=mysql_query($sql_huyen);
		$str='
			<select id="id_item" name="id_item"">
			<option>Chọn danh mục</option>			
...
<form name="frm" method="post" action="index.php?com=dichvu&act=save" enctype="multipart/form-data" class="nhaplieu">   	<b>Danh mục</b> <?=get_main_category()?><br />	
   <?php if (@$_REQUEST['act']=='edit')
	{?>
	<b>Hình hiện tại:</b><img src="<?=_upload_dichvu.$item['photo']?>" alt="NO PHOTO"  width="150"/><br />
	<?php }?>
    <br />
