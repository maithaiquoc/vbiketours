...
<script language="javascript">				
	...
	{				
		...
		window.location ="index.php?com=nhahang&act=<?php if($_REQUEST['act']=='edit') echo 'edit'; else echo 'add';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_cat="+a.value;	
		return true;
	}
...
function get_main_category()
	{
		$sql_huyen="select * from table_nhahang_cat order by id desc ";
...
function get_main_item()
	{
		$sql_huyen="select * from table_nhahang_item where id_cat='".$_REQUEST["id_cat"]."' order by id desc ";
		$result=mysql_query($sql_huyen);
		$str='
...
<form name="frm" method="post" action="index.php?com=nhahang&act=save" enctype="multipart/form-data" class="nhaplieu">   
   <?php if (@$_REQUEST['act']=='edit')
	{?>
	<b>Hình hiện tại:</b><img src="<?=_upload_nhahang.$item['photo']?>" alt="NO PHOTO"  width="150"/><br />
	<?php }?>
    <br />
  ...
  <b>Mô tả ( english )</b>
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=nhahang&act=man'" class="btn" />
</form>