...
function onSearch(evt) {	
		...
		location.href = "index.php?com=datphong&act=man&keyword="+keyword;
	...
	<tr>
        ...
        <td style="width:10%;"><?=$items[$i]['sodt']?></td>
         <td style="width:10%;"><?=$items[$i]['email']?></td>                                 
          <td style="width:10%;"><?=$items[$i]['ngayden']?></td>
         <td style="width:10%;"><?=$items[$i]['ngaydi']?></td>
         <?php
         //Xac sinh loai phong
		$d->reset();
		$sql_ks="SELECT`id`,`ten_vi`,`tenkhongdau` FROM #_khachsan where `hienthi`=1 and id=".$items[$i]['loaiphong']." ORDER BY stt,id ASC";
		$d->query($sql_ks);
		$result_ks=$d->fetch_array();
		?>
        <td style="width:10%;"><?=$result_ks['ten_vi']?></td>
         <td style="width:5%;"><?=$items[$i]['songuoi']?></td>
        <td style="width:5%;"><a href="index.php?com=datphong&act=delete&id=<?=$items[$i]['id']?>" onClick="if(!confirm('Xác nhận xóa')) return false;"><img src="media/images/delete.png" /></a></td>
	</tr>
	<?php	}?>
    </table>
