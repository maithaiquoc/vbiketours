...
<form name="frm" method="post" action="index.php?com=news&idproduct=<?=$item['id_new']?>&perentid=<?=$item['id']?>&act=save_reply" enctype="multipart/form-data" class="nhaplieu">	
<br />  
<b>Tin tức</b> <input type="text" value="<?=@$item['ten_vi']?>" class="input" />
...
<b>&nbsp;</b><input type="button" value="Lưu" onclick="onSubmit()" class="btn" /><input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=news&act=man_comments'" class="btn" />
...
<td style="width:5%;"><a href="index.php?com=news&act=delete_reply&perentid=<?=$items_comments[$i]['parentid']?>&id=<?=$items_comments[$i]['id']?>" onClick="if(!confirm('Xác nhận xóa')) return false;"><img src="media/images/delete.png" /></a></td>
</tr>
<?php	}?>
</table>
</form>